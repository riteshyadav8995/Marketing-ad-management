...
router.get('/funnel', async (req: Request, res: Response) => {
  try {
    ...
    const firstTouch: Record<string, any> = {
      organic: { leads: 0, customers: 0, revenue: 0 },
      paid_social: { leads: 0, customers: 0, revenue: 0 },
      paid_search: { leads: 0, customers: 0, revenue: 0 }
    };

    const lastTouch: Record<string, any> = {
      organic: { leads: 0, customers: 0, revenue: 0 },
      paid_social: { leads: 0, customers: 0, revenue: 0 },
      paid_search: { leads: 0, customers: 0, revenue: 0 }
    };

    const getChannel = (source: string | null) => {
      if (!source) return 'organic';
      const s = source.toLowerCase();
      if (s.includes('facebook') || s.includes('meta') || s.includes('instagram')) return 'paid_social';
      if (s.includes('google') || s.includes('adwords')) return 'paid_search';
      return 'organic';
    };
    ...
    for (const lead of leads) {
      ...
      if (isCustomer) firstTouch[ftChannel].customers += 1;
      ...
      if (isCustomer) lastTouch[ltChannel].customers += 1;
